...
class AdaHessian(Optimizer):
    ...
    def apply_gradients(self, grads_hess_and_vars, name=None, skip_gradients_aggregation=False, **kwargs,):
        ...
        with tf.name_scope(scope_name):
            ...
            hessians = self._clip_gradients(hessians)
            grads = self._deduplicate_sparse_grad(grads)
            hessians = self._deduplicate_sparse_grad(hessians)
    ...
    def _all_reduce_sum_gradients(self, grads_hess_and_vars):
        ...
        if filtered_grads_hessians_and_vars:
            if tf.__internal__.distribute.strategy_supports_no_merge_call():
                ...
                hessians = [pair[1] for pair in filtered_grads_hessians_and_vars]
                reduced_grads = tf.distribute.get_replica_context().all_reduce(
                    tf.distribute.ReduceOp.SUM, grads
                )
                reduced_hess = tf.distribute.get_replica_context().all_reduce(
                    tf.distribute.ReduceOp.SUM, hessians
                )
                reduced = [(g, h) for g, h in zip(reduced_grads, reduced_hess)]
            else:
                filtered_grads_and_vars = tuple([(g, v) for (g, h, v) in filtered_grads_hessians_and_vars])
                filtered_hessian_and_vars = tuple([(h, v) for (g, h, v) in filtered_grads_hessians_and_vars])
                reduced_grads = tf.distribute.get_replica_context().merge_call(
                    self._all_reduce_sum_fn, args=(filtered_grads_and_vars,)
                )
                reduced_hess = tf.distribute.get_replica_context().merge_call(
                    self._all_reduce_sum_fn, args=(filtered_hessian_and_vars,)
                )
                reduced = [(g, h) for g, h in zip(reduced_grads, reduced_hess)]
        else:
            reduced = []
        ...
        for g, h, v in grads_hess_and_vars:
            if g is None:
                ...
                reduced_with_nones.append((reduced[reduced_pos][0], reduced[reduced_pos][1], v))
                reduced_pos += 1
